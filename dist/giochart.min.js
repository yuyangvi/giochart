	        ...
	        if (type === "retention") {
	            return (ev) => {
	                ...
	                keys.forEach((key) => {
	                    ...
	                    if (interval !== null && Object.keys(chartConfig_1.RetentionCOT[interval]).includes(items[0].value)) {
	                        ev.items.push(lodash_1.assign({}, items[0], { value: items[1].value }, { name: chartConfig_1.RetentionCOT[interval][items[0].value] }));
	                    }
	                    else {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	exports.getTmFormat = (tmInterval) => {
	    if (tmInterval > 6048e5) {
	        ...
	        return (v) => (`${moment.unix(v / 1000).format("MM-DD ddd")} ~ ${moment.unix(v / 1000).endOf("month").format("MM-DD ddd")}`);
	    }
	    else if (tmInterval === 6048e5) {
	        return (v) => (`${moment.unix(v / 1000).format("MM-DD ddd")} ~ ${moment.unix(v / 1000).endOf("week").format("MM-DD ddd")}`);
	    }
	...
	exports.getAxisFormat = (tmInterval) => {
	    ...
	    if (tmInterval > 6048e5) {
	        return (n) => (month[parseInt(n.slice(0, 2), 10) - 1]);
	    }
	    else if (tmInterval === 6048e5) {
	...
	exports.getRetentionParams = (chartType, columns, params, isCOT) => {
	    ...
	    // 处理 turn的问题
	    if (isCOT && !isCompare) {
	        const grua = params.granularities;
	        const interval = lodash_1.find(grua, { id: "tm" }).interval;
	...
	class ContextListener extends React.Component {
	    render() {
	        ...
	        if (chartParams.chartType === "table") {
	            ...
	            const source = utils_1.retentionSourceSelector(this.context.source, ["comparison_value"], false, parseInt(lodash_1.find(chartParams.granularities, { id: "tm" }).interval, 10), this.props.timeRange);
	            let values = null;
