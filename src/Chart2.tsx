...
const getChartConfig: any = (chartType: string) => {
  ...
  const chartTypeMap: any[string] = {
    ...
    // retention: { geom: "line", size: 2, shape: "smooth", counter: "day", margin: [10, 30, 50, 40] },
    retention: { geom: "interval", counter: "day", shape: ["stroke", "hollowRect"], margin: [10, 30, 50, 40] },
...
class Chart extends React.Component <ChartProps, any> {
  ...
  private drawChart(chartParams: DrawParamsProps, source: any[], isThumb: boolean = false) {
    try {
      ...
      if (chartCfg.geom === "line" && (chartCfg.counter || chartCfg.skipMetric)) { // 留存点缀
      ...
      if (chartCfg.shape) {
        geom.shape("metric", chartCfg.shape);
      }
  ...
  private buildSourceConfig(chartParams: DrawParamsProps): SourceConfig {
    ...
    chartParams.columns.forEach((m: Metric) => {
      ...
      if (!m.isDim) {
        sourceDef[m.id].tickCount = 4;
      }
    });
    if (sourceDef.tm && chartParams.chartType !== "retention") {
      sourceDef.tm = {
        tickCount: 4,
